...
const std::vector<std::string> TASK_NAMES_HIGH_PRIO = {"servicemanag",
                                                       "zygote",
                                                       "writeback",
                                                       "kblockd",
                                                       "rcu_tasks_kthre",
                                                       ...
                                                       "thread_fence",
                                                       "Input"};
const std::vector<std::string> TASK_NAMES_LOW_PRIO = {"ipawq", "iparepwq",
                                                      "wlan_logging_th"};
const std::vector<std::string> TASK_NAMES_RT_FF = {
    "kgsl_worker_thread", "devfreq_boost", "mali_jd_thread",
    "mali_event_thread",  "crtc_commit",   "crtc_event",
    "pp_event",           "rot_commitq_",  "rot_doneq_",
    "rot_fenceq_",        "system_server", "surfaceflinger",
    "composer",           "fts_wq",        "nvt_ts_work"};
...
std::string executeCommand(const std::string &command,
  ...
  if (pipe) {
    ...
    logMessage("Executed: " + command + (status ? " [fail]" : " [ok]"),
               config::LOG_FILES.at(logKey));
  } else {
...
void setTaskAffinity(const std::string &namePattern,
                     const std::string &hexMask) {
...
void setThreadAffinity(const std::string &namePattern,
                       const std::string &threadPattern,
                       const std::string &hexMask) {
  for (pid_t pid : getProcessIDs(namePattern)) {
...
void setTaskNice(const std::string &namePattern, int niceVal) {
  for (pid_t pid : getProcessIDs(namePattern)) {
    for (pid_t tid : getThreadIDs(pid)) {
      std::string cmd =
          "renice -n " + std::to_string(niceVal) + " -p " + std::to_string(tid);
      executeCommand(cmd, "nice");
    }
  }
}

void setThreadNice(const std::string &namePattern,
                   const std::string &threadPattern, int niceVal) {
  for (pid_t pid : getProcessIDs(namePattern)) {
    for (pid_t tid : getThreadIDs(pid, threadPattern)) {
      std::string cmd =
          "renice -n " + std::to_string(niceVal) + " -p " + std::to_string(tid);
...
void setTaskRt(const std::string &namePattern, int priority) {
  for (pid_t pid : getProcessIDs(namePattern)) {
    for (pid_t tid : getThreadIDs(pid)) {
      std::string cmd =
          "chrt -p " + std::to_string(priority) + " " + std::to_string(tid);
      executeCommand(cmd, "rt");
    }
  }
}

void setTaskIoPrio(const std::string &namePattern, int classType,
                   int classLevel) {
  for (pid_t pid : getProcessIDs(namePattern)) {
    for (pid_t tid : getThreadIDs(pid)) {
      std::string cmd = "ionice -c " + std::to_string(classType) + " -n " +
                        std::to_string(classLevel) + " -p " +
                        std::to_string(tid);
...
void setIrqAffinity(const std::string &irqPattern, const std::string &hexMask) {
  std::string cmd = "grep -r '" + irqPattern +
                    "' /proc/interrupts | grep -oE '^[0-9]+' | sort -u";
...
// Pin only the first thread matching pattern to one mask, use another mask for
// the rest
void pinProcOnFirst(const std::string &taskPattern,
                    const std::string &hexMaskPrime,
                    const std::string &hexMaskOthers) {
...
void optimizeLauncher(const std::string &launcherPkg) {
  setThreadAffinity(launcherPkg, "RenderThread|GLThread",
                    config::RENDERTHREAD_MASK);
  setThreadAffinity(
      launcherPkg,
      "GPU completion|HWC release|hwui|FramePolicy|ScrollPolicy|ged-swd",
      config::GPUWORKERS_MASK);
}
...
void optimizeGraphics() {
  ...
  pinProcOnFirst("crtc_commit:", config::PERFCORE_MASK,
                 config::GPUWORKERS_MASK);
  setTaskAffinity("pp_event", config::PERFCORE_MASK);
  setTaskAffinity("mdss_fb|mdss_disp_wake|vsync_retire_work|pq@",
                  config::MIDCORE_MASK);
...
void optimizeTasks() {
  for (const auto &t : config::TASK_NAMES_HIGH_PRIO)
    setTaskNice(t, -15);
  for (const auto &t : config::TASK_NAMES_LOW_PRIO)
    setTaskNice(t, 0);
  ...
  for (const auto &t : config::TASK_NAMES_RT_IDLE)
    setTaskRt(t, 0);
  for (const auto &t : config::TASK_NAMES_IO_PRIO)
    setTaskIoPrio(t, 3, 0);
...
int main() {
  try {
    ...
    std::string cmd = "pm resolve-activity -a \"" + INTENT_ACTION + "\" -c \"" +
                      INTENT_CATEGORY +
                      "\" | grep packageName | head -1 | cut -d= -f2";
    std::string launcherPkg = executeCommand(cmd, "launcher");
    launcherPkg = launcherPkg.substr(0, launcherPkg.find('\n'));
    if (launcherPkg.empty()) {
      logMessage("Launcher package not found, skipping launcher optimizations.",
                 config::LOG_FILES.at("main"));
    } else {
    ...
    logMessage("Task Optimization executed successfully!",
               config::LOG_FILES.at("main"));
  } catch (const std::exception &e) {
    logMessage(std::string("Exception occurred: ") + e.what(),
               config::LOG_FILES.at("error"));
    return 1;
  }
  return 0;
}